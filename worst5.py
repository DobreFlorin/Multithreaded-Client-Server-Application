...
def is_string_in_list(input_string, list_to_check):
    ...
    Args:
        input_string (str): The string to check for
        list_to_check (list): The list to search in

    Returns:
        bool: True if string is in list, False otherwise
    """
    return input_string in list_to_check


def worst5(question):
    """
    Get the 5 worst performing states for a given question.

    Args:
        question (str): The question/measurement to analyze

    Returns:
        dict: Dictionary containing the 5 worst states and their values
    """
    ...
    #sorted_averages = averages.sort_values(by='Data_Value')
